...
jQuery(function () {
  ...
  THEME_CONFIG.document_dynamic_list = THEME_CONFIG.document_dynamic_list
    ? THEME_CONFIG.document_dynamic_list
        .split(",")
        .filter(item => {
          if (item) {
            return true;
  ...
  THEME_CONFIG.document_no_display = THEME_CONFIG.document_no_display
    ? THEME_CONFIG.document_no_display
        .split(",")
        .filter(item => {
          if (item) {
            return true;
  ...
  let vm = new Vue({
    ...
    methods: {
      ...
      resume() {
        ...
        load.confirm("确定要恢复主题的默认配置吗？", () => {
          ...
          /* 发起请求 */
          axios
            .get("/?default=1&private=" + that.data.document_private)
            .then(res => {
      ...
      postLink() {
        ...
        load.confirm("确定提交站点所有URL到百度站长平台吗？", () => {
          load.loading("正在提交");
          axios
            .get("/?submit=1&private=" + that.data.document_private)
            .then(res => {
              message.info(res.data.result);
            })
            .catch(e => {
      ...
      sitemap() {
        ...
        load.confirm("确定生成TXT站点地图？", () => {
          load.loading("正在生成");
          axios
            .get("/?sitemap=1&private=" + that.data.document_private)
            .then(res => {
              message.info(res.data.result);
            })
            .catch(e => {
    ...
    created() {
      let that = this;
      /*同步插件更新日志*/
      axios.get("https://weixin.nicen.cn/api/theme").then(res => {
        if (res.data.code) {
