...
$(function () {
  ...
  (function () {
    ...
    /*初始化加载后的首次进度*/
    progress.text(
      (((window.scrollY + main.get(0).clientHeight) / main.get(0).scrollHeight) * 100).toFixed(0) +
        "%"
    );
    ...
    $(window).on("scroll", function () {
       ...
       * */
      progress.text(
        (((window.scrollY + that.clientHeight) / that.scrollHeight) * 100).toFixed(0) + "%"
      );
    });
  ...
  (function () {
     ...
     * 右边内容栏的sticky效果
     * */
    window.computed = () => {}; //暴露到全局
    window.toFixed = () => {}; //暴露到全局
